package com.sulake.habbo.communication.demo
{
    ...
    public class HabboLoginDemoView extends EventDispatcher 
    {
        ...
        public static const var_441:String = "InitConnection";

        private const var_2879:String = "fuselogin";

        private var var_2885:HabboCommunicationDemo;
        private var _window:IWindowContainer;
        private var var_2886:SSOTicket;
        ...
        {
            this.var_2885.windowManager.removeWindow("habbo_login_window");
        }
        ...
        {
            var _loc1_:XmlAsset = ((this.var_2885 as Component).assets.getAssetByName("login_window") as XmlAsset);
            if (_loc1_ == null)
            {
                Logger.log("Could  not find embedded Window XML for Login Window, login_window");
                return;
            };
            this._window = (this.var_2885.windowManager.createWindow("habbo_login_window", "Login", HabboWindowType.var_185, HabboWindowStyle.var_156, (HabboWindowParam.var_814 | HabboWindowParam.var_157), new Rectangle(10, 10, 320, 240), null) as IFrameWindow);
            var _loc2_:XML = (_loc1_.content as XML);
            var _loc3_:IWindowContainer = (this.var_2885.windowManager.buildFromXML(_loc2_) as IWindowContainer);
            ...
            var _loc7_:SharedObject = SharedObject.getLocal(this.var_2879, "/");
            if (_loc5_ != null)
            {
            ...
            if (_loc8_)
            {
                if (this.var_2885.habboConfiguration)
                {
                    if (this.var_2885.habboConfiguration.getKey("use.sso", "false") == "true")
                    {
                        _loc8_.select();
                    };
                };
        ...
        public function populateUserList(param1:Map):void
        {
            ...
            var _loc3_:XmlAsset = (this.var_2885.assets.getAssetByName("login_user_list_item") as XmlAsset);
            var _loc4_:IWindow = this.var_2885.windowManager.buildFromXML((_loc3_.content as XML));
        ...
        private function windowEventProcessor(param1:WindowEvent=null, param2:IWindow=null):void
        {
            ...
            if (_loc3_ != null)
            {
                ...
                _loc6_ = SharedObject.getLocal(this.var_2879, "/");
            ...
            {
                dispatchEvent(new Event(var_441));
            };
        }
        ...
        private function listEventHandler(param1:WindowEvent, param2:IWindow):void
        {
            ...
            this.var_2885.sendTryLogin(this.name, this.password, param2.id);
        }

        private function initSSOTicket(param1:Boolean):void
        {
            if (this.var_2886 != null)
            {
                this.var_2886.removeEventListener(SSOTicket.var_1572, this.onParsedTicketSuccess);
                this.var_2886.removeEventListener(SSOTicket.var_1573, this.onParsedTicketFailure);
                this.var_2886 = null;
            };
            var _loc2_:String = "";
            if (this.var_2885.habboConfiguration != null)
            {
                _loc2_ = this.var_2885.habboConfiguration.getKey("url.prefix", _loc2_);
                _loc2_ = _loc2_.replace("http://", "");
                _loc2_ = _loc2_.replace("https://", "");
            };
            var _loc3_:IHabboWebLogin = this.var_2885.habboCommunication.habboWebLogin(this.name, this.password);
            this.var_2886 = new SSOTicket(this.var_2885.assets, _loc3_, _loc2_, param1);
            this.var_2886.addEventListener(SSOTicket.var_1572, this.onParsedTicketSuccess);
            this.var_2886.addEventListener(SSOTicket.var_1573, this.onParsedTicketFailure);
        }

        private function onParsedTicketSuccess(param1:Event):void
        {
            Logger.log(("Got ticket: " + this.var_2886.ticket));
            Logger.log(("Has Facebook: " + this.var_2886.isFacebookUser));
            if (this.var_2886.isFacebookUser)
            {
                this.var_2885.habboConfiguration.setKey("facebook.user", "1");
            };
            this.var_2885.flashClientUrl = this.var_2886.flashClientUrl;
            this.var_2885.ssoTicket = this.var_2886.ticket;
            this.var_2886.removeEventListener(SSOTicket.var_1572, this.onParsedTicketSuccess);
            this.var_2886.removeEventListener(SSOTicket.var_1573, this.onParsedTicketFailure);
            this.var_2886 = null;
            dispatchEvent(new Event(var_441));
