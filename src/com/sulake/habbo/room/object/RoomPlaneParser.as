package com.sulake.habbo.room.object
{
    ...
    public class RoomPlaneParser
    {
        ...
        private var _wallThickness: Number = 0;
        private var _floorThickness: Number = 0;
        private var _floorHeight: Number = 0;

        public function RoomPlaneParser()
        {
            this._wallHeight = 3.6;
            this._wallThickness = 0.25;
            this._floorThickness = 0.25;
        }
        ...
        public function setTileHeight(param1: int, param2: int, param3: Number): Boolean
        {
            ...
            {
                _loc4_ = (this._tilemap[param2] as Array);
                ...
                if (param3 >= 0)
                {
                    ...
                    if (param1 > this._maxX)
                    {
                        this._maxX = param1;
                    }

                    if (param2 < this._minY)
                    {
                        this._minY = param2;
                    }
                ...
                {
                    if (param1 == this._minX || param1 == this._maxX)
                    {
                        _loc5_ = false;
                        _loc6_ = this._minY;
                        while (_loc6_ < this._maxY)
                        {
                    ...
                    if (param2 == this._minY || param2 == this._maxY)
                    {
                        _loc7_ = false;
                        _loc8_ = this._minX;
                        while (_loc8_ < this._maxX)
                        {
        ...
        public function getTileHeight(param1: int, param2: int): Number
        {
            ...
            var _loc3_: Array = this._tilemap[param2] as Array;
        ...
        public function initializeFromTileData(param1: Boolean = false): Boolean
        {
            var _loc7_: Array;
            var _loc8_: int;
            var _loc2_: Point = findEntranceTile(this._tilemap);
            var _loc3_: int;
            if (_loc2_ != null)
            {
            ...
            this._floorHeight = getFloorHeight(this._tilemap);
            if (!param1)
            {
            ...
            var _loc5_: Array = [];
            var _loc6_: int;
            while (_loc6_ < this._tilemap.length)
            {
                _loc4_ = this._tilemap[_loc6_];
                _loc7_ = [];
                _loc8_ = 0;
        ...
        private function createWallPlanes(): Boolean
        {
            ...
            var _loc46_: Boolean;
            var _loc47_: Boolean;
            var _loc1_: Array = this._tilemap;
            if (_loc1_ == null)
            {
                return false;
            }
            ...
            var _loc10_: Array = [
                this.extractTopWall,
                this.extractRightWall,
                this.extractBottomWall,
                this.extractLeftWall
            ];
            var _loc11_: int;
        ...
        private function addWall(param1: IVector3d, param2: IVector3d, param3: IVector3d, param4: IVector3d, param5: Boolean, param6: Boolean, param7: Boolean): void
        {
            ...
            this.addPlane(RoomPlaneData.PLANE_WALL, param1, param2, param3, [param4]);
            this.addPlane(RoomPlaneData.PLANE_LANDSCAPE, param1, param2, param3, [param4]);
            var _loc8_: Vector3d;
            var _loc9_: Vector3d = Vector3d.crossProduct(param2, param3);
            var _loc10_: Vector3d = Vector3d.product(_loc9_, (1 / _loc9_.length) * -this._wallThickness);
            this.addPlane(RoomPlaneData.PLANE_WALL, Vector3d.sum(param1, param3), param2, _loc10_, [_loc9_, param4]);
            if (param5)
            {
                this.addPlane(RoomPlaneData.PLANE_WALL, Vector3d.sum(Vector3d.sum(param1, param2), param3), Vector3d.product(param3, -(param3.length + this._floorThickness) / param3.length), _loc10_, [
                    _loc9_,
                    param4
                ]);
            }
            ...
            if (param6)
            {
                this.addPlane(RoomPlaneData.PLANE_WALL, Vector3d.sum(param1, Vector3d.product(param3, -this._floorThickness / param3.length)), Vector3d.product(param3, (param3.length + this._floorThickness) / param3.length), _loc10_, [
                    _loc9_,
                    param4
                ]);
                if (param7)
                {
                    _loc11_ = Vector3d.product(param2, this._wallThickness / param2.length);
                    this.addPlane(RoomPlaneData.PLANE_WALL, Vector3d.sum(Vector3d.sum(param1, param3), Vector3d.product(_loc11_, -1)), _loc11_, _loc10_, [
                        _loc9_,
                        param2,
                        param4
                    ]);
        ...
        {
            if (param1 == null || this._tilemap == null || param1 == this._tilemap)
            {
                return false;
            }
            ...
            var _loc5_: Array;
            var _loc6_: Array;
            if (param1.length > this._tilemap.length)
            {
                return false;
            }
            ...
            while (_loc3_ < _loc7_)
            {
                ...
                _loc6_ = (this._tilemap[_loc3_] as Array);
        ...
        {
            var _loc12_: RoomPlaneData = this.addPlane(RoomPlaneData.PLANE_FLOOR, param1, param2, param3);
            ...
            if (_loc12_)
            {
                ...
                if (param6)
                {
                    ...
                    _loc14_ = this.addPlane(RoomPlaneData.PLANE_FLOOR, _loc13_, new Vector3d(0, 0, -this._floorThickness), param2);
                    if (_loc14_ != null)
                    {
                        if (param8 > 0)
                        {
                            _loc14_.addMask(0, 0, this._floorThickness, param8);
                        }

                        if (param9 > 0)
                        {
                            _loc14_.addMask(0, param2.length - param9, this._floorThickness, param9);
                        }
                ...
                if (param7)
                {
                    ...
                    _loc14_ = this.addPlane(RoomPlaneData.PLANE_FLOOR, _loc13_, new Vector3d(0, 0, -this._floorThickness), Vector3d.product(param2, -1));
                    if (_loc14_ != null)
                    {
                        if (param9 > 0)
                        {
                            _loc14_.addMask(0, 0, this._floorThickness, param9);
                        }

                        if (param8 > 0)
                        {
                            _loc14_.addMask(0, param2.length - param8, this._floorThickness, param8);
                        }
                ...
                if (param4)
                {
                    ...
                    _loc15_ = this.addPlane(RoomPlaneData.PLANE_FLOOR, _loc13_, new Vector3d(0, 0, -this._floorThickness), Vector3d.product(param3, -1));
                    if (_loc15_ != null)
                    {
                        if (param11 > 0)
                        {
                            _loc15_.addMask(0, 0, this._floorThickness, param11);
                        }

                        if (param10 > 0)
                        {
                            _loc15_.addMask(0, param3.length - param10, this._floorThickness, param10);
                        }
                ...
                if (param5)
                {
                    ...
                    _loc15_ = this.addPlane(RoomPlaneData.PLANE_FLOOR, _loc13_, new Vector3d(0, 0, -this._floorThickness), param3);
                    if (_loc15_ != null)
                    {
                        if (param10 > 0)
                        {
                            _loc15_.addMask(0, 0, this._floorThickness, param10);
                        }

                        if (param11 > 0)
                        {
                            _loc15_.addMask(0, param3.length - param11, this._floorThickness, param11);
                        }
        ...
        public function getPlaneLocation(param1: int): IVector3d
        {
            ...
            var _loc2_: RoomPlaneData = this._planes[param1] as RoomPlaneData;
            if (_loc2_ != null)
            {
                return _loc2_.loc;
            }
        ...
        public function getPlaneType(param1: int): int
        {
            if (param1 < 0 || param1 >= this.planeCount)
            {
                return RoomPlaneData.PLANE_UNKNOWN;
            }
        ...
        public function getPlaneMaskLeftSideLoc(param1: int, param2: int): Number
        {
            ...
            var _loc3_: RoomPlaneData = this._planes[param1] as RoomPlaneData;
            if (_loc3_ != null)
            {
