package com.sulake.habbo.avatar.animation
{
    ...
    public class Animation implements IAnimation 
    {
        ...
        private var var_2401:AvatarDataContainer;
        private var var_2396:Array = [];
        private var var_2402:DirectionDataContainer;
        private var var_2403:ShadowDataContainer;
        ...
        public function Animation(param1:AvatarStructure, param2:XML)
        {
            ...
            {
                this.var_2400 = String(param2.@desc);
            }
            else
            {
                this.var_2400 = this._id;
            };
            if (param2.hasOwnProperty("sprite"))
            {
                for each (_loc3_ in param2.sprite)
                {
                    this.var_2395.push(new SpriteDataContainer((this as IAnimation), _loc3_));
                };
            };
            if (param2.hasOwnProperty("avatar"))
            {
                this.var_2401 = new AvatarDataContainer(param2.avatar[0]);
            };
            if (param2.hasOwnProperty("canvas"))
            {
                for each (_loc4_ in param2.canvas)
                {
                    this.var_2396.push(new CanvasDataContainer(_loc4_));
                };
            };
            if (param2.hasOwnProperty("direction"))
            {
                this.var_2402 = new DirectionDataContainer(param2.direction[0]);
            };
            if (param2.hasOwnProperty("shadow"))
            {
                this.var_2403 = new ShadowDataContainer(param2.shadow[0]);
            };
            if (param2.hasOwnProperty("remove"))
            {
                for each (_loc5_ in param2.remove)
                {
                    this.var_2397.push(String(_loc5_.@id));
                };
            };
            if (param2.hasOwnProperty("add"))
            {
                ...
                {
                    this.var_2398.push(new AddDataContainer(_loc6_));
                };
            };
        ...
        public function getAnimatedBodyPartIds(param1:int, param2:String=null):Array
        {
            ...
            for each (_loc4_ in this.getFrame(param1, param2))
            {
                ...
                else
                {
                    ...
                    {
                        for each (_loc5_ in this.var_2398)
                        {
