package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
    ...
    public class UserDefinedRoomEventsHelp
    {
        ...
        private var var_4389: IWindowContainer;
        private var _window: IFrameWindow;
        private var _view: IWindowContainer;
        private var var_4390: int = 0;
        private var var_1364: Array;
        private var var_4391: IContainerButtonWindow;
        private var var_4392: IContainerButtonWindow;
        ...
        private function prepareWindow(): void
        {
            ...
            this._window = IFrameWindow(this.var_4378.getXmlWindow("ude_help"));
            this.var_4389 = IWindowContainer(this.find(this._window, "help_container"));
            this.var_4392 = (this.var_4389.findChildByName("button_previous") as IContainerButtonWindow);
            if (this.var_4392)
            {
                this.var_4392.procedure = this.onButtonClick;
            }

            this.var_4391 = (this.var_4389.findChildByName("button_next") as IContainerButtonWindow);
            if (this.var_4391)
            {
                this.var_4391.procedure = this.onButtonClick;
            }
        ...
        {
            if (this.var_4390 >= this.var_1364.length - 1)
            {
                this.var_4391.disable();
            }
            else
            {
                this.var_4391.enable();
            }

            if (this.var_4390 <= 0)
            {
                this.var_4392.disable();
            }
            else
            {
                this.var_4392.enable();
            }
        ...
        {
            this.var_4390++;
            this.var_4390 = Math.min(this.var_4390, this.var_1364.length - 1);
            this.openPage();
        }
        ...
        {
            this.var_4390--;
            this.var_4390 = Math.max(this.var_4390, 0);
            this.openPage();
        }
        ...
        private function initLocalizations(): void
        {
            ...
            this.var_1364 = [];
            var _loc1_: int = 1;
            while (this.var_4378.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
            {
                _loc2_ = this.var_4378.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
                _loc3_ = this.var_4378.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
                _loc4_ = this.var_4378.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
                this.var_1364.push([_loc2_, _loc3_, _loc4_]);
                _loc1_++;
            }
        ...
        private function localize(): void
        {
            ...
            if (!this.var_1364 || this.var_1364.length == 0)
            {
                return;
            }
            ...
            if (_loc1_)
            {
                _loc1_.text = ILocalization(this.var_1364[this.var_4390][0]).value;
            }

            if (_loc2_)
            {
                _loc2_.text = ILocalization(this.var_1364[this.var_4390][2]).value;
            }

            if (_loc3_)
            {
                _loc4_ = ILocalization(this.var_1364[this.var_4390][1]).value;
        ...
        {
            if (!param1 || !this.var_4378 || !this.var_4378.assets)
            {
                return;
            }

            var _loc2_: BitmapDataAsset = this.var_4378.assets.getAssetByName(param1) as BitmapDataAsset;
            if (_loc2_ == null)
            {
        ...
        private function retrievePreviewAsset(param1: String): void
        {
            ...
            var _loc2_: String = this.var_4378.configuration.getKey("image.library.catalogue.url");
            var _loc3_: * = _loc2_ + param1 + ".gif";
            Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
            var _loc4_: URLRequest = new URLRequest(_loc3_);
            var _loc5_: AssetLoaderStruct = this.var_4378.assets.loadAssetFromFile(param1, _loc4_, "image/gif");
            if (!_loc5_)
            {
                return;
            }
