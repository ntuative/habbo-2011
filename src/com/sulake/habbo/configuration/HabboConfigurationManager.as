package com.sulake.habbo.configuration
{
    ...
    public class HabboConfigurationManager extends Component implements IHabboConfigurationManager 
    {
        ...
        private var var_3377:Dictionary;
        private var var_2047:Boolean = false;
        private var var_3376:String = "";
        private var var_3378:Array = [];
        ...
        public function HabboConfigurationManager(param1:IContext, param2:uint=0, param3:IAssetLibrary=null)
        {
            ...
            this.var_2047 = ((param2 & HabboConfigurationFlags.var_421) > 0);
            this.var_3377 = new Dictionary();
            if (!this.var_2047)
            {
                lock();
            };
            ...
            this.var_3376 = this.getKey("url_prefix");
            if (((!(this.var_3376 == null)) && (this.var_3376.length > 0)))
            {
                this.setKey("client.fatal.error.url", (this.var_3376 + "/flash_client_error"));
            };
            this.init();
        }
        ...
        private function init():void
        {
            ...
            if (!this.var_2047)
            {
                ...
                _loc3_.addEventListener(AssetLoaderEvent.var_35, this.onInitConfiguration);
                _loc3_.addEventListener(AssetLoaderEvent.var_36, this.onConfigurationError);
            };
        }
        ...
        private function onConfigurationError(param1:Event=null):void
        {
            ...
            Core.error((((("Could not load external variables. URL " + this.getKey("external.variables.txt")) + "HTTP status ") + _loc3_) + ". Client startup failed!"), true, Core.var_84);
        }
        ...
        private function onInitConfiguration(param1:Event=null):void
        {
            ...
            if (((_loc4_) && (!(_loc4_ == ""))))
            {
                ...
                _loc6_.addEventListener(AssetLoaderEvent.var_35, this.onInitConfigurationOverride);
                _loc6_.addEventListener(AssetLoaderEvent.var_36, this.onConfigurationOverrideError);
            }
            else
            {
                if (!this.var_2047)
        ...
        private function parseConfiguration(data:String):void
        {
            ...
            this.var_3376 = this.getKey("url.prefix");
            if (this.var_3376.length > 0)
            {
                externalAccessDomain = this.var_3376.replace("http://", "");
        ...
        {
            var _loc2_:String = (this.var_3377[param1] as String);
            return (!(_loc2_ == null));
        }

        public function getKey(param1:String, param2:String="", param3:Dictionary=null):String
        {
            var _loc4_:String = (this.var_3377[param1] as String);
            if (_loc4_ == null)
            {
                _loc4_ = param2;
            };
            if (_loc4_ != null)
            {
                ...
                {
                    _loc4_ = _loc4_.replace("%predefined%", this.var_3376);
        ...
        public function setKey(param1:String, param2:String, param3:Boolean=false, param4:Boolean=false):void
        {
            if (param4)
            {
                if (this.var_3377[param1] == null)
                {
                    Logger.log(((param1 + "=") + param2));
                };
            };
            if (((this.var_3378.indexOf(param1) < 0) || (param3)))
            {
                this.var_3377[param1] = param2;
            };
            if (param3)
            {
                this.var_3378.push(param1);
            };
        }
