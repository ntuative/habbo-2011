package com.sulake.habbo.navigator
{
    ...
    public class IncomingMessages
    {
        ...
        private function onUserRights(param1: IMessageEvent): void
        {
            ...
            if (_loc2_.securityLevel >= SecurityLevelEnum.var_438)
            {
                this._navigator.data.eventMod = true;
            }

            if (_loc2_.securityLevel >= SecurityLevelEnum.var_895)
            {
        ...
        private function onRoomEnter(param1: IMessageEvent): void
        {
            ...
            {
                this._navigator.toolbar.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.var_145, HabboToolbarIconEnum.ROOMINFO));
        ...
        private function onRoomInfo(param1: IMessageEvent): void
        {
            ...
            else
            {
                ...
                {
                    if (_loc2_.data.doorMode == RoomSettingsData.DOOR_STATE_DOORBELL && this._navigator.sessionData.userName != _loc2_.data.ownerName)
                    {
                        this._navigator.doorbell.show(_loc2_.data);
                    }
                    else
                    {
                        if (_loc2_.data.doorMode == RoomSettingsData.DOOR_STATE_PASSWORD && this._navigator.sessionData.userName != _loc2_.data.ownerName)
                        {
        ...
        {
            var _loc1_: HabboToolbarSetIconEvent = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.var_176, HabboToolbarIconEnum.EXITROOM);
            var _loc2_: int = this.data == null ? 0 : this.data.homeRoomId;
            var _loc3_: int = this.data.enteredGuestRoom == null ? 0 : this.data.enteredGuestRoom.flatId;
            ...
            {
                if (this.data != null && this.data.homeRoomId > 0)
                {
                    _loc1_.iconState = "homeroom";
                }
                else
                {
                    _loc1_ = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.var_306, HabboToolbarIconEnum.EXITROOM);
                }
        ...
        private function onFlatCreated(param1: IMessageEvent): void
        {
            ...
            this._navigator.mainViewCtrl.reloadRoomList(Tabs.SEARCH_TYPE_MY_ROOMS);
        }
        ...
        private function onUserFlatCats(param1: IMessageEvent): void
        {
            ...
            var _loc3_: RoomsTabPageDecorator = RoomsTabPageDecorator(this._navigator.tabs.getTab(Tabs.TAB_ROOMS_PAGE).tabPageDecorator);
        ...
        private function onCanCreateRoom(param1: IMessageEvent): void
        {
            ...
            else
            {
                ...
                if (this._navigator.sessionData.hasUserRight("fuse_can_create_more_rooms", HabboClubLevelEnum.HC_LEVEL_VIP))
                {
        ...
        {
            this._navigator.send(param1
                                         ? new GetPublicSpaceCastLibsMessageComposer(param2)
                                         : new GetGuestRoomMessageComposer(param2, false, true));
        }
        ...
        {
            var view: SimpleAlertView;

            var parser: CantConnectMessageParser = (param1 as CantConnectMessageEvent).getParser();

            Logger.log("FAILED TO CONNECT: REASON: " + parser.reason);

            switch (parser.reason)
            {
                case CantConnectMessageParser.CANT_CONNECT_ROOM_FULL:
                    view = new SimpleAlertView(this._navigator, "${navigator.guestroomfull.title}", "${navigator.guestroomfull.text}");
                    view.show();

                    return;

                case CantConnectMessageParser.CANT_CONNECT_ROOM_QUEUE:
                    view = new SimpleAlertView(this._navigator, "${room.queue.error.title}", "${room.queue.error." + parser.parameter + "}");
                    view.show();

                    return;

                case CantConnectMessageParser.CANT_CONNECT_ROOM_BANNED:
                    view = new SimpleAlertView(this._navigator, "${navigator.banned.title}", "${navigator.banned.text}");
                    view.show();

                    return;

                default:
                    view = new SimpleAlertView(this._navigator, "${room.queue.error.title}", "${room.queue.error.title}");
                    view.show();
            }
