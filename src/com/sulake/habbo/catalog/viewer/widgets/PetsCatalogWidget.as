package com.sulake.habbo.catalog.viewer.widgets
{
    ...
    public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
    {
        ...
        override public function init(): Boolean
        {
            ...
            _loc2_.addEventListener(WindowEvent.var_590, this.onTextWindowEvent);
            ...
            this.var_2754 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
            if (this.var_2754 >= 8)
            {
                return false;
            }
            ...
            this.var_2756 = _loc3_.localizationId;
            this.var_2761 = [];
            if (page && page.viewer && page.viewer.catalog)
            {
                ...
                {
                    _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_2754, this);
                    if (_loc5_)
                    {
                        this.var_2761 = _loc5_.colors;
                        if (this.var_2761 == null)
                        {
                            this.var_2761 = [];
                        }
            ...
            this._offers.add(this.var_2754, _loc3_);
            events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT, this.onSelectProduct);
            events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX, this.onColourIndex);
            events.addEventListener(WidgetEvent.var_662, this.onApproveNameResult);
        ...
        private function onWidgetsInitialized(param1: CatalogWidgetsInitializedEvent = null): void
        {
            ...
            var _loc3_: Array = [];
            for each (_loc4_ in this.var_2761)
            {
        ...
        public function onDropMenuEvent(param1: WindowEvent, param2: IWindow): void
        {
            ...
            if (param1.type == WindowEvent.var_560)
            {
                ...
                if (this.var_2758 == null || _loc3_ >= this.var_2758.length)
                {
                    return;
                }

                this.var_2759 = _loc3_;
                this.updateImage();
            }
        ...
        private function onColourIndex(param1: CatalogWidgetColourIndexEvent): void
        {
            ...
            this.var_2760 = param1.index;
            if (this.var_2760 < 0 || this.var_2760 > this.var_2761.length)
            {
                this.var_2760 = 0;
            }
        ...
        {
            if (param1.productCode == this.var_2756)
            {
                this.var_2758 = this.parseSellableBreeds(param1.sellableBreeds);
                this.var_2759 = 0;
        ...
        private function parseSellableBreeds(param1: Array): Array
        {
            ...
            {
                if (_loc3_.type == this.var_2754 && _loc3_.sellable)
                {
        ...
        private function updateBreedSelections(): void
        {
            ...
            var _loc2_: Array = [];
            var _loc3_: int;
            while (_loc3_ < this.var_2758.length)
            {
                _loc5_ = (this.var_2758[_loc3_] as int);
                _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_2754, _loc5_), this.getRaceLocalizationKey(this.var_2754, _loc5_)));
                _loc3_++;
            }
        ...
        private function updateImage(): void
        {
            ...
            if (this.var_2758 == null || this.var_2759 >= this.var_2758.length)
            {
                return;
            }
            ...
            if (this.var_2760 >= 0 && this.var_2760 < this.var_2761.length)
            {
                _loc7_ = (this.var_2761[this.var_2760] as IPetColor);
                if (_loc7_ != null)
                {
                    _loc2_ = _loc7_.rgb;
                }
            ...
            var _loc3_: int = this.var_2758[this.var_2759] as int;
            var _loc4_: IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_2754, _loc3_, _loc2_, AvatarScaleType.var_106, this);
            if (_loc4_ == null)
            {
                return;
            }
            ...
            _loc4_.appendAction(AvatarAction.var_972, AvatarAction.var_965);
            _loc4_.endActionAppends();
            _loc4_.setDirection(AvatarSetType.var_136, 2);
            this.setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.var_136), true);
            _loc4_.dispose();
        ...
        private function getPurchaseParameters(): String
        {
            ...
            if (this.var_2760 >= this.var_2761.length)
            {
                return "";
            }

            var petColor: IPetColor = this.var_2761[this.var_2760] as IPetColor;
            var color: uint = petColor.rgb;
            var breed: int = this.var_2758[this.var_2759] as int;
            var p: String = this._name + String.fromCharCode(10) + breed + String.fromCharCode(10) + this.addZeroPadding(color.toString(16)
                                                                                                                                     .toUpperCase(), 6);
            return p;
        }
