package com.sulake.core.window.utils
{
    ...
    public class TextStyleManager
    {
        ...
        public static const var_1118: String = "regular";
        public static const ITALIC: String = "italic";
        public static const BOLD: String = "bold";
        private static var var_2309: Map;
        private static var var_2310: Array;
        private static var _eventDispatcher: IEventDispatcher;
        private static const var_2311: String = "{";
        private static const var_2312: String = "}";
        private static const var_2313: String = "/*";
        private static const var_2314: String = "*/";
        ...
        private static function init(): void
        {
            ...
            _loc1_.name = var_1118;
            _loc1_.color = 0;
            _loc1_.fontSize = "9";
            _loc1_.fontFamily = "Courier";
            _loc1_.fontStyle = "normal";
            _loc1_.fontWeight = "normal";
            var_2309[_loc1_.name] = _loc1_;
            var_2310.push(_loc1_.name);
            _loc1_ = new TextStyle();
            _loc1_.name = ITALIC;
            _loc1_.color = 0;
            _loc1_.fontSize = "9";
        ...
        {
            return var_2309[param1];
        }

        public static function getStyleWithIndex(param1: int): TextStyle
        {
            return var_2309.getWithIndex(param1);
        }

        public static function setStyle(param1: String, param2: TextStyle): void
        {
            var _loc3_: * = var_2309.getKeys().indexOf(param1) == -1;
            param2.name = param1;
            var_2309[param1] = param2;
            if (_loc3_)
            {
                var_2310.push(param1);
        ...
        public static function setStyles(param1: Array, param2: Boolean = false): void
        {
            ...
            if (param2)
            {
                _loc5_ = [var_2309[var_1118], var_2309[ITALIC], var_2309[BOLD]];
                var_2309.reset();
                var_2309[var_1118] = _loc5_[0];
                var_2309[ITALIC] = _loc5_[1];
                var_2309[BOLD] = _loc5_[2];
            }

            var _loc3_: int = var_2309.length;
            for each (_loc4_ in param1)
            {
                var_2309[_loc4_.name] = _loc4_;
                if (var_2310.indexOf(_loc4_.name) == -1)
                {
                    var_2310.push(_loc4_.name);
                }

            }

            _eventDispatcher.dispatchEvent(new Event(Event.CHANGE));
            if (var_2309.length != _loc3_)
            {
                _eventDispatcher.dispatchEvent(new Event(Event.ADDED));
            }
        ...
        {
            var _loc1_: Array = [];
            var _loc2_: int = var_2309.length;
            var _loc3_: int;
            while (_loc3_ < _loc2_)
            {
                _loc1_.push(var_2309.getWithIndex(_loc3_));
                _loc3_++;
            }
        ...
        private static function parseStyleNamesFromCSS(param1: String): Array
        {
            ...
            var _loc4_: Array = _loc3_.split(var_2312);
            if (countSubStrings(param1, var_2311) != countSubStrings(param1, var_2312))
            {
                throw new Error("Mismatching amount of \"" + var_2311 + "\" versus \"" + var_2312 + "\", please check the CSS!");
            }
            ...
            for each (_loc5_ in _loc4_)
            {
                while (true)
                {
                    if (_loc5_.indexOf(var_2313) == 0)
                    {
                        _loc5_ = _loc5_.substring(_loc5_.indexOf(var_2314) + 2, _loc5_.length);
                    }
                    else
                    {
                        break;
                ...
                _loc5_ = _loc5_.slice(0, _loc5_.indexOf(var_2311)).split(" ").join("");
                if (_loc5_.length)
                {
                    _loc2_.push(_loc5_);
                }
