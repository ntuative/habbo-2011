package com.sulake.core.window.graphics
{
    ...
    public class DrawBufferAllocator implements IDisposable 
    {
        ...
        public function get allocatedByteCount():uint
        {
            ...
            var bytes:uint;
            var index:uint = this.var_2245.length;
            try
            {
                while (index > 0)
                {
                    index--;
                    bitmap = BitmapData(this.var_2245[index]);
        ...
        public function dispose():void
        {
            ...
            {
                this._disposed = true;
                _loc1_ = this.var_2245.length;
                while (_loc1_ > 0)
                {
                    BitmapData(this.var_2245.pop()).dispose();
                    _loc1_--;
                };
            };
        }
