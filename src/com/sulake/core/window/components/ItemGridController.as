package com.sulake.core.window.components
{
    ...
    public class ItemGridController extends ItemListController implements IItemGridWindow 
    {
        ...
        protected function resolveColumnForNextItem(param1:IWindow):IItemListWindow
        {
            ...
            if (_loc2_ > 0)
            {
                ...
                if (_loc6_)
                {
                    ...
                    {
                        if ((_loc4_.rectangle.right + param1.width) <= var_1018.width)
                        {
                            return (this.addColumnForItem(param1));
                        };
                    };
        ...
        {
            var _loc2_:IItemListWindow = (_context.create(((_name + "_COLUMN_") + numListItems), null, WindowType.var_851, WindowStyle.var_1114, (WindowParam.var_693 | WindowParam.var_694), new Rectangle(0, 0, Math.max(param1.width, 0), Math.max(param1.height, 0)), this.listEventProc, null, numListItems, null, [WindowController.TAG_INTERNAL, WindowController.TAG_EXCLUDE]) as IItemListWindow);
