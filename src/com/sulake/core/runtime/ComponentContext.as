package com.sulake.core.runtime
{
    ...
    public class ComponentContext extends Component implements IContext
    {
        ...
        protected var var_2177: Array;
        protected var _debug: Boolean = false;
        protected var _displayObjectContainer: DisplayObjectContainer;

        public function ComponentContext(ctx: IContext, flags: uint = 0, assetLibrary: IAssetLibrary = null)
        {
            super(ctx, flags | Component.COMPONENT_FLAG_CONTEXT, assetLibrary);
        ...
        public function warning(message: String): void
        {
            ...
            {
                events.dispatchEvent(new WarningEvent(Component.COMPONENT_EVENT_WARNING, message));
            }
        ...
        public function error(message: String, fatal: Boolean, category: int = -1, err: Error = null): void
        {
            _lastError = message;
            ...
            {
                events.dispatchEvent(new ErrorEvent(Component.COMPONENT_EVENT_ERROR, message, fatal, err
                        ? err.getStackTrace()
                        : null));
            }
        ...
        final public function loadFromFile(param1: URLRequest, param2: LoaderContext): LibraryLoader
        {
            ...
            var _loc3_: uint;
            while (_loc3_ < this.var_2177.length)
            {
                if (this.var_2177[_loc3_].url == param1.url)
                {
                    return this.var_2177[_loc3_].loadFromFile(param1);
                }
        ...
        protected function loadErrorHandler(param1: LibraryLoaderEvent): void
        {
            ...
            this.removeLibraryLoader(_loc2_);
            this.error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_, true, Core.ERROR_CATEGORY_DOWNLOAD_RESOURCE_FAILED);
        }
        ...
        protected function removeLibraryLoader(param1: LibraryLoader): void
        {
            ...
            param1.dispose();
            var _loc2_: uint;
            while (_loc2_ < this.var_2177.length)
            {
                if (this.var_2177[_loc2_] == param1)
                {
                    this.var_2177.splice(_loc2_, 1);
                    return;
                }
            ...
            while (i < componentList.length())
            {
                ...
                if (assetCount > 0)
                {
                    ...
                    while (k < assetCount)
                    {
                        ...
                        {
                            this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!", true, Core.ERROR_CATEGORY_DOWNLOAD_CRITICAL_ASSET);
                        }
                ...
                if (componentClass == null)
                {
                    this.error("Invalid component class " + componentString + "!", true, Core.ERROR_CATEGORY_DOWNLOAD_CRITICAL_ASSET);
                    return null;
                }

                component = assetLibrary == null
                        ? new componentClass(this, flags)
                        : new componentClass(this, flags, assetLibrary);
                if (component != null)
                {
                    if (assetLibrary != null)
                    {
                        if (component.assets != assetLibrary)
                        {
                            ...
                            this.error("Component \"" + componentString + "\" did not save provided asset library!", true, Core.ERROR_CATEGORY_DOWNLOAD_CRITICAL_ASSET);
                        }
        ...
        final public function attachComponent(param1: Component, param2: Array): void
        {
            ...
            if (param1.locked)
            {
                param1.events.addEventListener(INTERNAL_EVENT_UNLOCKED, this.lockEventHandler);
            }
        ...
        final public function detachComponent(param1: Component): void
        {
            ...
            while (_loc5_ < this._components.length)
            {
                if (this._components[_loc5_] == param1)
                {
                    ...
                    param1.events.removeEventListener(INTERNAL_EVENT_UNLOCKED, this.lockEventHandler);
                    return;
                }
        ...
        final protected function announceInterfaceAvailability(param1: IID, param2: Component): void
        {
            ...
            while (_loc6_ < _loc5_)
            {
                ...
                if (_loc4_ == null)
                {
                    this.error("Interface " + param1 + " still unavailabe!", true, Core.ERROR_CATEGORY_INTERFACE_UNAVAILABLE);
                }
        ...
        override public function dispose(): void
        {
            ...
            if (!disposed)
            {
                ...
                if (this.var_2177 != null)
                {
                    _loc1_ = this.var_2177.length;
                    while (_loc1_-- > 0)
                    {
                        _loc2_ = this.var_2177[0];
                        this.removeLibraryLoader(_loc2_);
                        _loc2_.dispose();
                    }
        ...
        private function lockEventHandler(param1: LockEvent): void
        {
            ...
            if (param1.type == INTERNAL_EVENT_UNLOCKED)
            {
                _loc2_ = (param1.unknown as Component);
                _loc2_.events.removeEventListener(INTERNAL_EVENT_UNLOCKED, this.lockEventHandler);
                if (!disposed)
                {
                    _loc3_ = [];
